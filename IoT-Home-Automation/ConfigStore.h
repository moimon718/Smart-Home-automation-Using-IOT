...
#define EEPROM_CUSTOM_START 256
#define EEPROM_V0_ADDR EEPROM_CUSTOM_START
#define EEPROM_V1_ADDR (EEPROM_CUSTOM_START + 1)
#define EEPROM_V2_ADDR (EEPROM_CUSTOM_START + 2)
#define EEPROM_V3_ADDR (EEPROM_CUSTOM_START + 3)  // New
#define EEPROM_V4_ADDR (EEPROM_CUSTOM_START + 4)  // New
...
inline void saveV0(uint8_t v0) {
  EEPROM.write(EEPROM_V0_ADDR, v0);
  EEPROM.commit();
}

inline void saveV1(uint8_t v1) {
  EEPROM.write(EEPROM_V1_ADDR, v1);
  EEPROM.commit();
}

inline void saveV2(uint8_t v2) {
  EEPROM.write(EEPROM_V2_ADDR, v2);
  EEPROM.commit();
}

inline void saveV3(uint8_t v3) {  // New
  EEPROM.write(EEPROM_V3_ADDR, v3);
  EEPROM.commit();
}

inline void saveV4(uint8_t v4) {  // New
  EEPROM.write(EEPROM_V4_ADDR, v4);
  EEPROM.commit();
}

inline void loadSavedStates(uint8_t& v0, uint8_t& v1, uint8_t& v2, uint8_t& v3, uint8_t& v4) {
  v0 = EEPROM.read(EEPROM_V0_ADDR);
  v1 = EEPROM.read(EEPROM_V1_ADDR);
  v2 = EEPROM.read(EEPROM_V2_ADDR);
  v3 = EEPROM.read(EEPROM_V3_ADDR);
  v4 = EEPROM.read(EEPROM_V4_ADDR);
